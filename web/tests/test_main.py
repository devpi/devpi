...
def test_index_projects_arg(monkeypatch, tmpdir):
    ...
    n2n = {u'foo': u'foo'}
    monkeypatch.setattr(
        devpi_server.extpypi.PyPIStage, "init_pypi_mirror", lambda s, p: None)
    monkeypatch.setattr(
        devpi_server.extpypi.PyPIStage, "name2serials", n2s, raising=False)
    monkeypatch.setattr(
        devpi_server.extpypi.PyPIStage, "normname2name", n2n, raising=False)
